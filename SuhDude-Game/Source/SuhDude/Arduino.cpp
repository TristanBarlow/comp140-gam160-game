...
FString AArduino::ArduinoReadResult(FString RawDataRead)
{
	FString Var = RawDataRead;
	int32 foo = Var.Find(TEXT("s"));
	int32 bar = Var.Find(TEXT("s"), ESearchCase::Type::IgnoreCase , ESearchDir::Type::FromStart, foo);
	FString NewString = Var.RightChop(bar);
	NewString = NewString.LeftChop(foo);
